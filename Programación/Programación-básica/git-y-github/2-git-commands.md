...
- `git config`: 
  ...
  - `--local`: permite acceder la **configuración local** (i.e del repositorio).
    ...
    - `-l` o `--list`:  Muestra la configuración de **Git**.
      - `show`: **flags**.
...
- `stash`: Mueve las modificaciones de **working** y **staging** en un área temporal llamada **stash** permitiendo recuperarlas (justo en las áreas que se encontraban). Esto permite guardar cambios que no están listos para un **commit**.
  ...
  - `branch <nombre-del-branch>`: Crea una **rama** y mueve el **stash**
...
- `branch`: muestra las **ramas** del repositorio (marca con un `*` el **HEAD**).
  ...
  - `r`: Muestra las **ramas** del **servidor**
...
> [!IMPORTANT] El orígen
> 
> El **remote** es necesario para los **comandos de alojamiento**. Ver [cómo establecer remotes]()
...
- `pull`: `fetch` + `merge` 
  - `<remote> <nombre del branch>`: Respecto a la **rama**, descarga sus **commits** alojados en el **servidor** que no están _localmente_ y aplica un **merge**.
